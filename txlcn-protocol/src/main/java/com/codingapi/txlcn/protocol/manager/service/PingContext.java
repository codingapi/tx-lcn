...
public class PingContext {
  ...
  public boolean handlePong(final String thisServerName, final Pong pong) {
    ...
    if (pongs.containsKey(pongServerName)) {
      LOGGER
          .debug("{} from {} is already handled for {}", pong, pongServerName, ping.getPeerName());
      return false;
    }

    pongs.put(pongServerName, pong);

    LOGGER.debug("Handling {} from {} for {}. Pong #: {}", pong, pongServerName, ping.getPeerName(),
        pongs.size());
    ...
    if (!thisServerName.equals(ping.getPeerName())) {
      if (connection != null) {
        ...
        if (next != null) {
          LOGGER.debug("Forwarding {} to {} for initiator {}", pong, connection.getPeerName(),
              ping.getPeerName());
          connection.send(next);
        } else {
          LOGGER.error("Invalid {} received from {} for {}", pong, pongServerName,
              ping.getPeerName());
        }
      } else {
        LOGGER.error("No connection is found in ping context for {} from {} for {}", pong,
            pongServerName,
            ping.getPeerName());
      }
    }
  ...
  public boolean isTimeout() {
    return ping.getPingStartTimestamp() + ping.getPingTimeoutDurationInMillis() <= System
        .currentTimeMillis();
  }
