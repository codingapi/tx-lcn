...
/**
 * Maintains all of the ongoing Ping operations either initiated by this peer or other peers in the
 * network
 */
public class ForwardPingService implements IPingService {
  ...
  public ForwardPingService(ConnectionService connectionService,
  ...
  /**
   * Sends a keep-alive message to its neighbours to notify its presence. It sends {@link Ping}
   * messages to others instead of keep-alive messages at frequency given with {@link
   * Config#autoDiscoveryPingFrequency}
   *
   * @param discoveryPingEnabled if true, then peer is allowed to trigger a discovery ping to learn
   * about other peers
   */
  @Override
  ...
  /**
   * Initiates a Ping operation in the network. If there is an ongoing Ping operation, it only
   * attaches to it without starting a new Ping.
   *
  ...
  /**
   * Handles a Ping operation initiated by another node. If the received {@link Ping} message is
   * allowed to be propagated, this peer also propagates it to its own neighbours. Additionally, it
   * sends a {@link Pong} message back to the neighbour that has sent the {@link Ping} message to
   * this peer.
   *
   ...
   * @param ping Ping message that is received. It can be initiated by the neighbour with the given
   * connection or any other peer that has no direct connection to this peer
   */
  @Override
  public void handlePing(final InetSocketAddress bindAddress, final Connection connection,
      final Ping ping) {
    ...
    if (pingPeerName.equals(connection.getPeerName())) {
      ...
      LOGGER.info("Handling {} of initiator {} and forwarder {} with ttl={} and hops={}", ping,
          pingPeerName,
  ...
  /**
   * Handles a {@link Pong} message that is sent in response to a {@link Ping} message propagated by
   * this peer.
   *
   * @param pong Pong message that is sent in response to a {@link Ping} message propagated by this
   * peer.
   */
  @Override
  public void handlePong(final Pong pong) {
    ...
    if (pingContext == null) {
      LOGGER.warn("No ping context is found for {} from {} for initiator {}", pong,
          pong.getPeerName(),
          pingPeerName);
      return;
    }
  ...
  /**
   * Removes {@link Pong} messages sent by the disconnected peer to cancel out its presence in
   * ongoing Ping operations.
   *
    ...
    while (pingIt.hasNext()) {
      ...
      if (pingContext.removePong(disconnectedPeerName)) {
        ...
        if (pingOwnerConnection != null) {
          LOGGER.info("Removed pong of {} in ping of {}. Forwarding {} to {}", disconnectedPeerName,
              pingPeerName,
      ...
      for (Pong pong : new ArrayList<>(pingContext.getPongs())) {
        if (pong.getSenderPeerName().equals(disconnectedPeerName)) {
          ...
          rePing = true;
          LOGGER
              .info("Removed Pong of {} in ping of {} since it was sent by {}", pong.getPeerName(),
                  pingPeerName,
          ...
          if (connection != null) {
            ...
            connection.send(msg);
            LOGGER.info("Forwarded {} to {} for ping of {}", msg, connection.getPeerName(),
                pingPeerName);
          }
        }
      }
        ...
        if (next != null) {
          ...
          for (Connection connection : connectionService.getConnections()) {
            if (!(connection.getPeerName().equals(pingPeerName) || connection.getPeerName()
              ...
              connection.send(next);
              LOGGER.info("{} re-sent to {} because {} left", next, connection.getPeerName(),
                  disconnectedPeerName);
            }
          }
        }
  ...
  /**
   * Cancels out Ping operations of the disconnected peer. Additionally, neighbours that sent {@link
   * Pong} messages to the cancelled Ping are also notified.
   *
  ...
  public void cancelPings(final Connection connection, final String disconnectedPeerName) {
    ...
    while (pingIt.hasNext()) {
      ...
      boolean shouldRemove =
          pingPeerName.equals(disconnectedPeerName) && connection.equals(pingOwnerConnection);
      if (!shouldRemove) {
        shouldRemove = pingOwnerConnection != null && pingOwnerConnection.getPeerName()
            .equals(disconnectedPeerName);
      }
      ...
      if (shouldRemove) {
        ...
        for (Pong pong : pingContext.getPongs()) {
          ...
          if (c != null) {
            ...
            LOGGER.warn("{} not sent to {} since there is no connection", cancelPings,
                pong.getPeerName());
          }
        }
      }
  ...
  /**
   * Terminates ongoing Ping operations that completed the timeout duration. If there is a
   * terminated Ping operation initiated by this peer, {@link Pong} messages collected are
   * returned.
   *
    ...
    while (pingIt.hasNext()) {
      ...
      if (pingContext.isTimeout()) {
        ...
        if (config.getPeerName().equals(pingPeerName)) {
          ...
          LOGGER.info("Ping for {} has timed out. Notifying futures with # peers: {}",
              pingContext.getPeerName(),
              peers.size());
          for (CompletableFuture<Collection<String>> future : pingContext.getFutures()) {
