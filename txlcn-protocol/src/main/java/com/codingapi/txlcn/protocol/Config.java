...
public class Config {
  ...
  /**
   * When a peer has less number of connections than min number of active connections, it pings
   * other peers periodically and try to connect to new peers
   */
  private int minNumberOfActiveConnections = DEFAULT_MIN_NUMBER_OF_ACTIVE_CONNECTIONS;
  /**
   * When a peer does not send any message for the specified amount of seconds, it will be
   * disconnected
   */
  private int maxReadIdleSeconds = DEFAULT_MAX_READ_IDLE_SECONDS;
  /**
   * when a peer does not send ant message for the specified amount of seconds.it will send
   * heartbeat message
   */
  private int maxWriteIdleSeconds = DEFAULT_MAX_WRITE_IDLE_SECONDS;
  /**
   * Amount of time that a peer will send periodic keep-alive messages to its neighbours to indicate
   * that it is alive
   */
  ...
  /**
   * When a peer automatically sends keep-alive messages periodically, it makes the {@code
   * autoDiscoveryPingFrequency}th a full ping
   */
  private int autoDiscoveryPingFrequency = DEFAULT_AUTO_DISCOVERY_PING_FREQUENCY;
  /**
   * When a peer starts an election, it waits until leader election timeout duration passes. If
   * there is no rejection message is received until timeout, peer announces itself as leader.
   */
